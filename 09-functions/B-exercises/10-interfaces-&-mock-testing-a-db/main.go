...
// MockDatastore is a temporary service that stores retrievable data in a map.
type MockDatastore struct {
	Users map[int]User
}

// attached to MockDatastore are 2 methods

func (md MockDatastore) GetUser(id int) (User, error) {
	user, ok := md.Users[id] // lookup on a map
	if !ok {
...
func (md MockDatastore) SaveUser(u User) error {
	_, ok := md.Users[u.ID] // lookup on a map
	if ok {                 // ensure we aren't overwriting
...
// -- `*sql.DB`
type Service struct {
	ds Datastore // store a DataStore
}

// the following 2 methods will call the respective functions for each underlying type, either MockDataStore or *sql.DB
...
func main() {
	// create a McokDataStore
	db := MockDatastore{
		Users: make(map[int]User),
	}

	// create a service, this time for MockDataStore
	srvc := Service{
		ds: db,
	}
